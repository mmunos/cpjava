		...
		while(T-->0) {
			...
			for(char c: s) {
				if(c == 'X') {
					...
					c_left_x = (c_left_x + c_left_f + c_left_o + 1) % mod;
					c_left_f = 0; c_left_o = 0;
					...
					c_left_o = (c_left_x + c_left_f + c_left_o + 1) % mod;
					c_left_x = 0; c_left_f = 0;
					...
					if(qf == -1) {
						r_left_x = (r_left_x + (c_left_f * currf % mod) + r_left_x) % mod;
						r_left_x = (r_left_x + (c_left_x * currx % mod)) % mod;
					}
					else if(qf == 1){
						r_left_o = (r_left_o + (c_left_f * currf % mod) + r_left_o) % mod;
						r_left_o = (r_left_o + (c_left_o * curro % mod)) % mod;
					}
					...
					if(qf == -1) {
						currf = (currf + currx) % mod;
						curro = (curro + currx) % mod;
						currx = currx * 2 % mod;
					}
					else if(qf == 1){
						currf = (currf + curro) % mod;
